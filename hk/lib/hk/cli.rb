...
Successfully loaded and validated #{results[:loaded_templates].size} template(s):")
  ...
  class CLI < Thor
    ...
    def scan(target)
      ...
      bar_total = loaded_templates.size; bar = options[:json] ? nil : TTY::ProgressBar.new("Executing templates [:bar] :current/:total :percent :etas", total: bar_total, clear: true, output: $stderr); all_findings = []; all_execution_errors = [] # Progress bar to $stderr
...
Errors during template execution (#{all_execution_errors.size}):"); all_execution_errors.each_with_index do |err_info, idx| if err_info.is_a?(Hash) && err_info[:error]; error_message = "Error ##{idx + 1}: "; error_message += "Request Index: #{err_info[:request_index]} - " if err_info[:request_index]; error_message += "#{err_info[:error]}"; error_message += " (URL: #{err_info[:url]})" if err_info[:url]; puts "    #{error_message}"; else; puts "    Error ##{idx + 1}: #{err_info}"; end; end; end
    ...
    # Updated 'ports' command as per current task description (turn 163)
    ...
    option :timeout, type: :numeric, desc: "Per-port connect timeout in seconds (default: 1.0)"
    option :threads, type: :numeric, aliases: "-T", banner: "N", desc: "Number of concurrent threads for port scanning (default: 10)" # Added threads option
    def ports(target)
      ...
      if cli_options[:top_ports]
        top_n_list = [80, 443, 22, 21, 25, 53, 3306, 3389, 8080, 8443, 110, 143, 5432, 5900, 6379, 9200, 9300, 27017]; count = cli_options[:top_ports].to_i; parsed_ports = top_n_list.take(count > 0 ? count : 10)
        puts pastel.dim("  (Using top #{parsed_ports.size} ports based on --top-ports #{cli_options[:top_ports]})")
      elsif cli_options[:ports]
        ports_string = cli_options[:ports]; ports_string.split(',').each do |part| part.strip!; if part.include?('-'); start_port, end_port = part.split('-').map(&:to_i); if start_port && end_port && start_port > 0 && end_port >= start_port && end_port <= 65535 && start_port <=65535; parsed_ports.concat((start_port..end_port).to_a); else; puts pastel.yellow("Warning: Invalid port range '#{part}'. Skipping."); end; else; port = part.to_i; if port > 0 && port <= 65535; parsed_ports << port; else; puts pastel.yellow("Warning: Invalid port number '#{part}'. Skipping."); end; end; end; parsed_ports.uniq!.sort!
      ...
      if parsed_ports.empty?; puts pastel.red("Error: No valid ports specified or derived. Use -p or --top-ports."); return; end
      
      scan_options = { 
        timeout: options[:timeout] || 1.0,
        threads: options[:threads] # Pass threads to scanner options
      }.compact # Remove nil options
      
      # puts pastel.dim("CLI options for scan: #{scan_options.inspect}")
      # puts pastel.dim("Effective ports to be scanned (#{parsed_ports.size}): #{parsed_ports.inspect}")

      net_scanner = HK::Net::Scanner.new
      
      bar_total = parsed_ports.empty? ? 0 : parsed_ports.size
      progress_bar = bar_total > 0 ? TTY::ProgressBar.new("Scanning #{target} [:bar] :current/:total (:percent) :etas", total: bar_total, clear: true, output: $stderr) : nil
      scan_options[:progress_bar] = progress_bar # Pass bar instance in options

      scan_results = net_scanner.tcp_scan(target, parsed_ports, scan_options)
      progress_bar&.finish 
      
      puts pastel.cyan("CLI: Scan Results for ") + pastel.yellow.bold(target)
      if scan_results[:error]; puts pastel.red("  Error: #{scan_results[:error]}"); return; end
      if scan_results[:open_ports].any?; puts pastel.green("  Open Ports:"); scan_results[:open_ports].each do |pi| details = "    Port #{pi[:port]}"; details += " - Service: #{pastel.bright_blue(pi[:service])}" if pi[:service] && pi[:service] != "unknown"; details += " (Version: #{pastel.blue(pi[:version])})" if pi[:version]; puts pastel.green(details); end; else; puts pastel.yellow("  No open ports found from the scanned list."); end
      if scan_results[:filtered_ports].any? ; puts pastel.yellow("  Filtered/Timed Out Ports: ") + scan_results[:filtered_ports].join(', '); end
    end
  ...
  Errors during crawl (#{results[:errors].size}):"); results[:errors].each_with_index do |err_info, idx| if err_info.is_a?(Hash) && err_info[:error]; error_message = "Error ##{idx + 1}: "; error_message += "Request Index: #{err_info[:request_index]} - " if err_info[:request_index]; error_message += "#{err_info[:error]}"; error_message += " (URL: #{err_info[:url]})" if err_info[:url]; puts "    #{error_message}"; else; puts "    Error ##{idx + 1}: #{err_info}"; end; end; end; puts "--------------------------------------------------"; puts pastel.cyan("Scan finished.")
    ...
    def subdomains(target_domain_or_url) # Condensed
      finder_options = { timeout: options[:timeout] }.compact; begin; finder = HK::SubdomainFinder.new(target_domain_or_url, finder_options); rescue ArgumentError => e; puts pastel.red("Error: #{e.message}"); return 1; end; discovered_subdomains = finder.discover; if discovered_subdomains.empty?; puts pastel.yellow("No subdomains found for #{finder.domain}."); else; puts pastel.green("Found #{discovered_subdomains.size} unique subdomain(s) for #{finder.domain}:"); if options[:output]; begin; output_dir = File.dirname(options[:output]); FileUtils.mkdir_p(output_dir) unless File.directory?(output_dir); File.open(options[:output], 'w') do |file| discovered_subdomains.each { |sd| file.puts sd } end; puts pastel.green("Subdomain list saved to: #{options[:output]}"); rescue SystemCallError => e ; puts pastel.red("Error saving subdomains to file #{options[:output]}: #{e.message}"); puts pastel.yellow("Printing to console instead:"); discovered_subdomains.each { |sd| puts "  #{pastel.bright_blue(sd)}" }; end; else; discovered_subdomains.each { |sd| puts "  #{pastel.bright_blue(sd)}" }; end; end; return 0
    end
