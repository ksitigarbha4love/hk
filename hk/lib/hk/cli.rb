...
require 'fileutils'      
require 'json' # Added for JSON output
require_relative '../hk' 
...
Successfully loaded and validated #{results[:loaded_templates].size} template(s):")
  ...
  class CLI < Thor
    ...
    # Updated 'scan' command for JSON output
    ...
    long_desc <<-LONGDESC
      ...
      using the -t/--templates option. Output can be saved to JSON using --json.

      Example:
        hk scan example.com -t templates/web_vulns/ --json results.json
    LONGDESC
    ...
    def scan(target)
      ...
      # Initial CLI output (not suppressed by JSON option yet)
      ...
      # 1. Load Templates
      # Suppress "Loading templates..." if JSON output is active, or make it part of a verbose mode later.
      puts pastel.magenta("Loading templates...") unless options[:json]
      HK::TemplateRegistry.clear! 
      load_results = template_engine.load_from_path(options[:templates])

      # Loading errors are important even for JSON output, print to STDERR or include in JSON.
      # For now, let's print to STDOUT if not JSON, or STDERR if JSON.
      if load_results[:errors].any?
        error_output_stream = options[:json] ? $stderr : $stdout
        error_output_stream.puts pastel.yellow("Encountered errors during template loading:")
        load_results[:errors].each { |err| error_output_stream.puts pastel.yellow("  - #{err}") }
      end

      loaded_templates = load_results[:loaded_templates]
      if loaded_templates.empty?
        message = "No templates were successfully loaded. Aborting scan."
        options[:json] ? $stderr.puts(pastel.red(message)) : puts(pastel.red(message))
        return
      end
      puts pastel.green("Successfully loaded #{loaded_templates.size} template(s).") unless options[:json]
      puts "--------------------------------------------------" unless options[:json]

      # 2. Execute Templates
      puts pastel.magenta("Executing templates against #{target_url}...") unless options[:json]
      
      bar_total = loaded_templates.size
      bar = options[:json] ? nil : TTY::ProgressBar.new("Executing templates [:bar] :current/:total :percent :etas", total: bar_total, clear: true)
      ...
      # bar&.finish # Not strictly needed if clear: true and bar completes
      puts "--------------------------------------------------" unless options[:json]

      # 3. Process and Output Results
      if options[:json]
        json_output_path = options[:json]
        output_data = {
          target_info: {
          ...
          summary: {
            templates_loaded: loaded_templates.size,
            findings_count: all_findings.size,
            execution_errors_count: all_execution_errors.size
          },
          findings: all_findings.sort_by { |f| sev_sort_order(f[:severity]) }, # Sort findings
          errors: all_execution_errors
        }
        begin
          File.write(json_output_path, JSON.pretty_generate(output_data))
          puts pastel.green("Scan results saved to JSON: #{json_output_path}")
        rescue SystemCallError, IOError => e
          puts pastel.red("Error: Could not write JSON output to #{json_output_path}: #{e.message}")
        end
      else # Standard pretty STDOUT display
        if all_findings.any?
  ...
  Severity: #{severity_color(severity&.to_s || 'unknown')}")
              findings_by_severity.each_with_index do |finding, idx|
                  puts "    Finding ##{idx + 1}:"; puts "      Template Name: #{finding[:template_name]} (#{finding[:template_id]})"; puts "      Target:        #{finding[:target_url]}"; puts "      Matched At:    #{finding[:matched_at_url]}"; puts "      Description:   #{finding[:description]}"
              end
          end
        else
...
Errors during template execution (#{all_execution_errors.size}):")
          all_execution_errors.each_with_index do |err_info, idx| 
            if err_info.is_a?(Hash) && err_info[:error]; error_message = "Error ##{idx + 1}: "; error_message += "Request Index: #{err_info[:request_index]} - " if err_info[:request_index]; error_message += "#{err_info[:error]}"; error_message += " (URL: #{err_info[:url]})" if err_info[:url]; puts "    #{error_message}"; else; puts "    Error ##{idx + 1}: #{err_info}"; end
          end
        end
        puts "--------------------------------------------------"
    ...
    # --- Other commands (ports, http, crawl - condensed for brevity) ---
    ...
    def ports(target) # ... (implementation from turn 162) ...
    ...
    def http(url) # ... (implementation from turn 156) ...
    ...
    def crawl(url) # ... (implementation from turn 156) ...
